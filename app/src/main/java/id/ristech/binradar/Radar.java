...
public class Radar {
    ...
    public void draw(Canvas canvas) {
        ...
        Orientation.ORIENTATION orient = Orientation.ORIENTATION.LANDSCAPE;
        if (AugmentedReality.useRadarAutoOrientate) {
            orient = ARData.getDeviceOrientation();
            if (orient== Orientation.ORIENTATION.PORTRAIT) {
                canvas.save();
                canvas.translate(0, canvas.getHeight());
                canvas.rotate(-90);
            } else if (orient== Orientation.ORIENTATION.PORTRAIT_UPSIDE_DOWN) {
                canvas.save();
                canvas.translate(canvas.getWidth() - ui_ud_pad, 0);
                canvas.rotate(90);
            } else if (orient== Orientation.ORIENTATION.LANDSCAPE_UPSIDE_DOWN) {
                canvas.save();
        ...
        if (orient!= Orientation.ORIENTATION.LANDSCAPE)
            canvas.restore();
    }
